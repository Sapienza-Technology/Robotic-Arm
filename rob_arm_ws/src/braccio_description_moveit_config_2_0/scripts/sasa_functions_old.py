...
def inv_kin_total_bis(position:np.array, Rtarg:np.matrix, phase, elbow:bool, prec_q:np.array)->np.array:
    prec_qfin = prec_q[3:]
    prec_qin  = prec_q[:3]
...
def range_check(q:np.array):
  ...
  if q[0]<-pi or q[0]>pi:      
    res = False
    bool_vec[0] = 0
  if q[1]<-pi/18 or q[1]>pi:
    res = False
    bool_vec[1] = 0
  if q[2]<-17*pi/18 or q[2]>-pi/12:
    res = False
    bool_vec[2] = 0
  return res, bool_vec
...
def out_of_joint_range(all_q:np.array, elbow:bool, coord:np.array):
  best=np.array([2*pi, 2*pi, 2*pi])
  high_limits = np.array([pi, pi, -pi/12])         #limiti
  low_limits = np.array([-pi, -pi/18, -17*pi/18])       #limiti
  for i in range(8):
    q0=all_q[0][i]
    q1=all_q[1][i]
    q2=all_q[2][i]
...
def go_straight(task:bool, elbow:bool, q_pres:np.array, direct: string, steps:int, distance, in_final_frame:bool = False):
    ...
    for i in range(steps):
      ...
      #print(coord)
      prec_q = q_pres
      q_des.append(inv_kin_total_bis(coord, rot, task, elbow, prec_q))
    return q_des
